...
def process_train_data(train_data_df):
    ...
    Y_train = train_data_encode[:, 0].astype(int)
...
def process_data(train_data_path, test_data_path = "", test_size=0.2):
    if test_data_path == "":
        ...
        X_train, Y_train, continuous_index, discrete_index = process_train_data(train_df)
        _, X_test, _, Y_test = train_test_split(X_train, Y_train, test_size=test_size)
...
if __name__ == '__main__':
    ...
    plt.savefig("./ieee_fig_new/Mushroom_ieee_acc.png")
    plt.show()
    ...
    plt.legend(loc="lower right")
    plt.savefig("./ieee_fig_new/Mushroom_ieee_precision.png")
    plt.show()
    ...
    plt.ylabel("F1 Score")
    plt.legend(loc="lower right")
    plt.savefig("./ieee_fig_new/Mushroom_ieee_f1.png")
    plt.show()
