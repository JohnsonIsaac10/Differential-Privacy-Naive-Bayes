...
def process_data(train_data_df):
    ...
    continuous_index = []  # 连续属性的索引
    discrete_index = []  # 离散属性的索引
    label_encoder = []  # 标签编码器列表
...
class DataAggregator:
    ...
    def calc_mean_variance_lap(self, attr_pert, attr_pert_square):
        ...
        mean_square_class = [mean_square_ / p_class for mean_square_, p_class in
                             zip(mean_square, self.prior_pert_proba)]
    ...
    def calc_mean_variance(self, attr_pert, attr_pert_square, attr_test, attr_test_square):
        ...
        mean_test_class = [mean_test_in_class / p_class for mean_test_in_class, p_class in
                           zip(mean_test, self.prior_pert_proba)]

        mean_test_square = np.mean(attr_test_square, axis=0)
        mean_test_square_class = [mean_square_ / p_class for mean_square_, p_class in
                                  zip(mean_test_square, self.prior_pert_proba)]
    ...
    def calc_mean_variance_multi(self, sample_collect, sample_square_collect):
        ...
        for i in range(sample_collect.shape[0]):
            ...
            for j in range(len(variance_class)):
                if variance_class[j] < 0:
                    variance_class[j] = -variance_class[j]
...
class NaiveBayesLDP:
    ...
    def fit_multi_continuous(self, X_train, Y_train):
        ...
        for i in range(X_train.shape[0]):
            sample = X_train[i, :]
            sample_pert, sample_pert_square = self.individuals.Multi_PM_continuous(sample=sample, label=Y_train[i],
                                                                                   length=length)
    ...
    def fit(self, X_train, Y_train, continuous_index):
        ...
        for i in range(attr_num):
            ...
            if i in continuous_index:
                # 连续属性
                attr_pert, attr_pert_square, attr_test, attr_test_square = self.individuals.PM_continuous_attr1(attr,
                                                                                                                Y_train)
                ...
                attr_count = len(set(attr))  # 某一属性可能的取值数
                vec_pert_attr, attr_count, binary_vec = self.individuals.SUE_map_discrete_attr(attr=attr,
                                                                                               Y_data=Y_train)
                self.dataAggregator.calc_num_of_attr(vec_pert_attr=vec_pert_attr, attr_count=attr_count,
